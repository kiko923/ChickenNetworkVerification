...
<body>
          ...
          <div class="layui-card-body">
            ...
            <script type="text/html" id="test-table-toolbar-toolbarDemo">
              <div class="layui-btn-container">
                <button class="layui-btn layui-btn-sm" lay-event="show"><i class="layui-icon layui-icon-refresh"></i>刷新</button>
              </div>
            </script>
  ...
  layui.config({
    ...
    table.render({
      ...
      ,url: '../interface/ajax.php?t=agent&a=heartbeat-json'
    ...
    table.on('tool(test-table-toolbar)', function(obj){
      ...
      if(obj.event === 'del'){
        layer.confirm('确认强制下线['+data.user+']用户吗？<br><span style="font-size:10px;color:red;">强制下线后用户将在下次心跳后离线</span>',{title:'二次确认'}, function(index){
            $.ajax({
                type:'POST',
                url:'../interface/ajax.php?t=agent&a=heartbeat-del&id='+data.id,
                dataType: 'json',
                data: '',
                cache: false,
