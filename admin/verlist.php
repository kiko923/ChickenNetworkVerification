...
<body>
  ...
  layui.config({
    ...
    table.render({
      ...
      ,url: '../interface/ajax.php?t=admin&a=ver-json&id='+idd
      ...
      ,cols: [[
        ...
        ,{field:'current_ver', title:'旧版本号', width:100, align:'center'}
        ,{field:'new_ver', title:'新版本号', width:100, align:'center'}
        ,{field:'type', title:'更新方式', width:100, align:'center'}
        ,{field:'update_time', title:'更新时间', width:180, align:'center'}
        ,{field:'update_url', title:'更新地址', minWidth:300}
    ...
    table.on('toolbar(test-table-toolbar)', function(obj){
      ...
      switch(obj.event){
              ...
              ,yes: function(index, layero){
                ...
                iframeWindow.layui.form.on('submit(LAY-user-front-submit)', function(datas){
                  var field = datas.field;
                    $.ajax({
                        type:'POST',
                        url:'../interface/ajax.php?t=admin&a=ver-set',
                        dataType: 'json',
                        data: field,
                        cache: false,
        ...
        case 'del':
            ...
            layer.confirm('确认删除选中版本吗？',{title:'二次确认'}, function(index){
            $.ajax({
                type:'POST',
                url:'../interface/ajax.php?t=admin&a=ver-delor',
                dataType: 'json',
    ...
    table.on('tool(test-table-toolbar)', function(obj){
      ...
      if(obj.event === 'del'){
        layer.confirm('确认删除选中版本吗？',{title:'二次确认'}, function(index){
            $.ajax({
                type:'POST',
                url:'../interface/ajax.php?t=admin&a=ver-del&id='+data.id,
                dataType: 'json',
                data: '',
                cache: false,
                success: function (data) {
        ...
        layer.open({
		  title:'修改版本[id:'+data.id+']'
          ,type: 2
          ,content: 'verinfo.php?id='+data.id
          ,shadeClose: true
          ,area: admin.screen() < 2 ? ['100%', '100%'] : ['695px', '80%']
		  ,btn: ['确定', '取消']
          ,maxmin: true
          ,yes: function(index, layero){
            ...
            iframeWindow.layui.form.on('submit(LAY-user-front-submit)', function(datas){
              var field = datas.field;
                $.ajax({
                    type:'POST',
                    url:'../interface/ajax.php?t=admin&a=ver-set&id='+data.id,
                    dataType: 'json',
                    data: field,
                    cache: false,
                    success: function (data) {
