...
<div class="layui-form layui-form-pane" lay-filter="layuiadmin-form-role" id="layuiadmin-form-role" style="margin: 15px;">
    ...
    <div class="layui-form-item">
        <label class="layui-form-label">制卡人员</label>
        <div class="layui-input-block">
            <input type="text" name="agent" placeholder="选填" autocomplete="off" class="layui-input">
        </div>
    </div>
    <div class="layui-form-item">
  ...
  layui.config({
    ...
    $.ajax({
      ...
      form.on('select(applist)', function(data){
          var load = layer.load();
          $.ajax({
              url: '../interface/ajax.php?t=admin&a=get-cardtype&id='+data.value,
              type: 'POST',
              dataType: 'html',
              data: '',
              success: function (res) {
      ...
      form.on('select(cardtype)', function(data){
          var load = layer.load();
          $.ajax({
              url: '../interface/ajax.php?t=admin&a=get-cardtypeinfo&id='+data.value,
              type: 'POST',
              dataType: 'html',
              data: '',
              success: function (res) {
