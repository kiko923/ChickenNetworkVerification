...
$row = DB::query('SELECT a.*,b.name as appname,b.dl_type as dl_type,c.user as user,c.endtime as endtime,c.point as point FROM `ty_heartbeat` as a left join `ty_app` as b on a.appid = b.id left join `ty_user` as c on a.uid=c.id where c.aid='.$userinfo['id'].' order by a.id limit '.($_GET['page'] - 1) * $_GET['limit'].','.$_GET['limit']);
    if(!$row){
        json(201,'暂无数据');
    }
    $jsondata['code'] = 200;
