...
if($myurl != $_SERVER['SERVER_NAME']){ //拦截未通过认证的调用
    ...
    $row = DB::query('SELECT a.*,b.name as appname,b.dl_type,d.name as gname,d.color as color FROM `ty_user` as a left join `ty_app` as b on a.appid = b.id left join `ty_usergroup` as d on a.gid=d.id'.whereStr($where).' order by a.id limit '.($_GET['page'] - 1) * $_GET['limit'].','.$_GET['limit']);
    if(!$row){
        json(201,'暂无数据');
    }
    $jsondata['code'] = 200;
    $jsondata['msg'] = '';
    $jsondata['count'] = count(DB::query('SELECT count(a.*) FROM `ty_user` as a left join `ty_app` as b on a.appid = b.id left join `ty_usergroup` as d on a.gid=d.id'.whereStr($where).' order by a.id'));
