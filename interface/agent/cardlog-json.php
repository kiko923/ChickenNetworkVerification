...
$infos = isset($_GET['infos']) ? purge($_GET['infos']) : '';
    ...
    $jsondata['code'] = 200;
    $jsondata['msg'] = '';
    $jsondata['count'] = count(DB::query('SELECT a.*,b.dl_type as dl_type,b.name as appname,c.user as user,d.user as agent FROM `ty_cardlog` as a left join `ty_app` as b on a.appid = b.id left join `ty_user` as c on a.uid=c.id left join `ty_admin` as d on a.aid=d.id'.whereStr($where).' order by a.id desc'));
