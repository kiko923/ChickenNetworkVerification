...
function random($length = 25,$lx = 0,$lx2 = NULL){//随机码
    if($lx == 0){
...
<div class="panel panel-primary">
	<div class="panel-heading" style="background: #15A638;">
		<h3 class="panel-title" align="center">炸鸡网络验证系统</h3>
	</div>
...
if(defined("SAE_ACCESSKEY") || $_GET['jump']==1){
	...
	if(!DB_USER || !DB_PASSWD || !DB_NAME) {
		...
		if(!$con = DB::connect(DB_HOST,DB_USER,DB_PASSWD,DB_NAME,DB_PORT)){
			...
			if(DB::query("select * from ".DB_PRE."core where 1")==FALSE)
				echo '<p align="right"><a class="btn btn-primary btn-block" href="?do=4">创建数据表>></a></p>';
			else
				echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过炸鸡网络验证系统</div>
	...
	$db_qz=isset($_POST['db_qz'])?$_POST['db_qz']:'ty_';
...
?>";
		if(!$con=DB::connect($db_host,$db_user,$db_pwd,$db_name,$db_port)){
			...
			echo '<div class="alert alert-success">数据库配置文件保存成功！</div>';
			if(DB::query("select * from ".$db_qz."core where 1")==FALSE)
				echo '<p align="right"><a class="btn btn-primary btn-block" href="?do=4">创建数据表>></a></p>';
			else
				echo '<div class="list-group-item list-group-item-info">系统检测到你已安装过炸鸡网络验证系统</div>
				<div class="list-group-item">
...
if(!DB_USER||!DB_PASSWD||!DB_NAME) {
    ...
    DB::query("INSERT INTO `".DB_PRE."admin`(user,type) VALUES ('admin', 0)");
	DB::query("INSERT INTO `".DB_PRE."core`(config_key,config_value) VALUES ('build', '".$date."')");
	DB::query("INSERT INTO `".DB_PRE."core`(config_key,config_value) VALUES ('sysid', '".random(20,0,1)."')");
	DB::query("INSERT INTO `".DB_PRE."core`(config_key,config_value) VALUES ('sysmac', '".random(8)."')");
}
if($e==0) {
